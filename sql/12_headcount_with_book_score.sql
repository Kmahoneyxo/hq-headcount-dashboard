-- Headcount model + Book Building FY26 score (query 10 + sales_book_summary_v2)
-- Dashboard source query — run this to refresh docs/data/headcount.json
--
-- Book score: avg of FY26 account flags from sales_book_summary_v2 (latest time_period)
-- See sql/11_book_score_join.sql for score methodology detail
...
-- Book Building FY26 score (sales_book_summary_v2)
...
SELECT
  ...
  bsm.reps_scored AS reps_with_fy26_book_score,
  CASE
    WHEN ROUND(ma.assigned_accounts * 1.0 / NULLIF(ma.current_reps, 0), 0) < pb.perfect_book_accounts * 0.90
      AND bsm.avg_pct_book_built < 50 THEN 'Grow books + improve FY26 score'
    WHEN ROUND(ma.assigned_accounts * 1.0 / NULLIF(ma.current_reps, 0), 0) < pb.perfect_book_accounts * 0.90
      THEN 'Grow books toward perfect book'
    WHEN bsm.avg_pct_book_built < 50 THEN 'Improve FY26 book build score'
    ELSE 'On track'
  END AS book_score_action
