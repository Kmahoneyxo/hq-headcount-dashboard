-- Growth-by-bucket export companion to sql/16_dashboard_export.sql
-- One row per country × segment × PCID bucket with median quarterly rev growth.
-- Join into headcount.json as growth_by_bucket[] per market (see scripts/json-from-mcp-results.py).
