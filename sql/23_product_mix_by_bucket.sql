-- Product mix (CPC vs CPA revenue share) by PCID bucket — companion to sql/16_growth_by_bucket_export.sql
-- One row per country × segment × bucket: median rep-level CPC % of total revenue (current 90d).
-- Join into headcount.json as product_mix_by_bucket[] (scripts/merge-bucket-exports.py).
...
rep_level AS (
  SELECT
    ...
    SUM(CASE WHEN j.dl__yyyymmdd_cst BETWEEN '20260427' AND '20260725'
      THEN j.cpc_revenue_millicents ELSE 0 END) / 100000.0 AS cpc_revenue_90d,
    SUM(CASE WHEN j.dl__yyyymmdd_cst BETWEEN '20260427' AND '20260725'
      THEN j.cpa_revenue_millicents ELSE 0 END) / 100000.0 AS cpa_revenue_90d,
...
rep_filtered AS (
  SELECT
    ...
    cpc_revenue_90d,
    cpa_revenue_90d,
    cpc_revenue_90d / NULLIF(cpc_revenue_90d + cpa_revenue_90d, 0) AS cpc_share,
    cpa_revenue_90d / NULLIF(cpc_revenue_90d + cpa_revenue_90d, 0) AS cpa_share,
    CASE
  ...
  WHERE revenue_prior >= 5000
    AND (cpc_revenue_90d + cpa_revenue_90d) > 0
)
...
SELECT
  ...
  ROUND(APPROX_PERCENTILE(cpc_share, 0.5), 3) AS median_cpc_share,
  ROUND(APPROX_PERCENTILE(cpa_share, 0.5), 3) AS median_cpa_share,
  ROUND(APPROX_PERCENTILE(cpc_revenue_90d, 0.5), 0) AS median_cpc_revenue_90d,
  ROUND(APPROX_PERCENTILE(cpa_revenue_90d, 0.5), 0) AS median_cpa_revenue_90d
FROM rep_filtered
