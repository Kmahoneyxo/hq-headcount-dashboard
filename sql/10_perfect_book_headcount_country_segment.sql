-- Perfect book + optimal headcount by country × segment
-- Goal 1: Find the book-size threshold where revenue growth peaks (diminishing returns after)
-- Goal 2: Calculate optimal headcount = total addressable accounts / perfect book size
--
-- Method:
--   1. Bucket reps by accounts_per_rep (fine-grained buckets)
--   2. Compute median capped revenue growth per bucket (segment × country)
--   3. Perfect book = largest bucket whose median growth is within 90% of the peak
--      AND growth in the next bucket is lower (diminishing returns confirmed)
--   4. Optimal headcount = (assigned accounts + assignable whitespace) / perfect book midpoint
--
...
-- Growth curve by segment × country × bucket
bucket_growth AS (
  SELECT
    segment,
    country,
    ...
    ROUND(AVG(revenue_growth_pct), 3) AS avg_growth_pct
  FROM rep_filtered
  GROUP BY 1, 2, 3, 4, 5, 6
  HAVING COUNT(DISTINCT sales_rep_id) >= 5
),

-- Peak growth per segment × country
peak AS (
  SELECT
    segment,
    country,
    MAX(median_growth_pct) AS peak_growth_pct
  FROM bucket_growth
  GROUP BY 1, 2
),

-- Next-bucket growth for diminishing returns check
with_next AS (
  SELECT
    b.*,
    p.peak_growth_pct,
    LEAD(b.median_growth_pct) OVER (
      PARTITION BY b.segment, b.country ORDER BY b.bucket_order
    ) AS next_bucket_growth,
    LEAD(b.bucket_order) OVER (
      PARTITION BY b.segment, b.country ORDER BY b.bucket_order
    ) AS next_bucket_order
  FROM bucket_growth b
  JOIN peak p ON b.segment = p.segment AND b.country = p.country
),

-- Perfect book = highest bucket still near peak AND next bucket shows decline
perfect_book AS (
  SELECT
    segment,
    country,
    book_bucket AS perfect_book_bucket,
    bucket_midpoint AS perfect_book_accounts,
    bucket_upper AS perfect_book_max,
    median_growth_pct AS perfect_book_growth_pct,
    rep_count AS reps_in_perfect_bucket
  FROM (
    SELECT *,
      ROW_NUMBER() OVER (
    ...
    FROM with_next
    WHERE median_growth_pct >= peak_growth_pct * 0.90  -- within 10% of peak
      AND (
        next_bucket_growth IS NULL                          -- largest bucket
        OR next_bucket_growth < median_growth_pct           -- diminishing returns
        OR next_bucket_order IS NULL
      )
...
-- Assigned accounts + current reps by country × segment
market_accounts AS (
  SELECT
    p.company_size_segment AS segment,
    COALESCE(m.market, REGEXP_EXTRACT(j.current_sales_team_name, '^([A-Z]{2})-', 1)) AS country,
...
-- SBS whitespace (segment-global pool)
sbs AS (
  SELECT
    p.company_size_segment AS segment,
...
SELECT
  pb.segment,
  pb.country,
  -- Perfect book (diminishing returns threshold)
  ...
  ROUND(
    (ma.assigned_accounts + COALESCE(s.unassigned_accounts, 0) * 0.10)
    / NULLIF(pb.perfect_book_accounts, 0),
    0
  ) AS optimal_headcount_with_10pct_whitespace,
  ...
  CASE
    WHEN ma.current_reps < ROUND(ma.assigned_accounts * 1.0 / NULLIF(pb.perfect_book_accounts, 0), 0) * 0.90
      AND pb.perfect_book_growth_pct > 0
      THEN 'Hire'
    WHEN ma.current_reps > ROUND(ma.assigned_accounts * 1.0 / NULLIF(pb.perfect_book_accounts, 0), 0) * 1.10
      THEN 'Optimize'
    WHEN pb.perfect_book_growth_pct <= 0
      THEN 'Do Not Hire'
    ELSE 'Hold'
  END AS headcount_recommendation,
  s.unassigned_accounts AS sbs_whitespace_segment
FROM perfect_book pb
JOIN market_accounts ma
  ON pb.segment = ma.segment AND pb.country = ma.country
LEFT JOIN sbs s ON pb.segment = s.segment
ORDER BY ma.revenue_90d DESC;
