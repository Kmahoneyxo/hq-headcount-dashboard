-- Impact coverage by PCID bucket — companion to sql/16_growth_by_bucket_export.sql
-- Median impact_calls_per_account per country × segment × bucket (reps with impact_calls > 0).
-- Join into headcount.json as coverage_by_bucket[] (scripts/merge-bucket-exports.py).
