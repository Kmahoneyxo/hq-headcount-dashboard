-- Rep-level book profile — ALL reps (export / audit trail)
-- Same logic as sql/17_rep_book_profile.sql but no too_big/too_little filter.
...
-- Run on Quest prod; merge via scripts/merge-rep-book.py → rep_book.json
...
rep_book AS (
  SELECT
    rb.segment, rb.country, rb.sales_rep_id, rb.sales_team_name,
    rb.pcid_count, rb.pqr_90d, rb.revenue_90d,
    rb.impact_calls_90d, ROUND(rb.impact_calls_per_account, 2) AS impact_calls_per_account,
    pb.ideal_pcid,
